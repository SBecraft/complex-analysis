...
- This directory sits inside the `ai-defense-labs` umbrella repo and is **its own independent repository**, linked into the parent as a **git submodule**. The parent pins a specific commit of this repo, so work committed here does not appear in the umbrella until someone runs `git add complex-analysis && git commit` there. The umbrella showing this submodule as modified is expected, not a problem to fix.
